...
facerec = dlib.face_recognition_model_v1(r"C:\Users\manya\OneDrive\Desktop\ITSP\dlib_face_recognition_resnet_model_v1.dat")
detector = dlib.get_frontal_face_detector()
predictor = dlib.shape_predictor(r"C:\Users\manya\OneDrive\Desktop\ITSP\shape_predictor_68_face_landmarks.dat")

# Initialize face descriptors and labels
face_descriptors = {}
labels = []
...
input_requested = False  # Flag to track if input is requested
...
# Function to load face descriptors from images in directory
...
# Load face descriptors from the directory
...
def compare_faces(known_face_descriptors, face_descriptor, tolerance=0.5):
    ...
    for label, descriptors in known_face_descriptors.items():
        if descriptors:  # Check if descriptors is not empty
    ...
    while capture_running:
        ...
        if not success:
            ...
            for face in faces:
                ...
                # Calculate the new starting point to keep the center the same
                x_new = max(0, x - w // 2)
                y_new = max(0, y - h // 2)
                ...
                if label == "Not Recognized":
                    ...
                    if current_time - last_unrecognized_time >= 10:
                        if frame_queue.qsize() < 1:  # Limit queued photos to 1
                            unrecognized_face = frame[y_new:y_new+h, x_new:x_new+w].copy()  # Make a copy without stopping capture
                            last_unrecognized_time = current_time
                            event_queue.put("unrecognized")  # Signal unrecognized face
                        else:
...
def index():
    ...
    input_requested = True  # Set input_requested to True when accessing the index page
    ...
    if input_requested and name:
        ...
        if unrecognized_face is not None:
            ...
            if len(faces) == 1:
                ...
                # Save updated face descriptors and labels
